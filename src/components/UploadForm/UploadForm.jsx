...
function UploadForm({ backendURL }) {
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    name: "",
    stripeToken: "" // We will store the Stripe token here after payment
  });
  const [hasPaid, setHasPaid] = useState(false);
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);

  const [userId, setUserId] = useState(null); // Define the state for userId
  const [showConfirmationMessage, setShowConfirmationMessage] = useState(false); // Define state for showConfirmationMessage
  ...
  const handleSubmit = async (e) => {
    ...
    // First, we upload the image file to S3 and update the state with the returned URL.
    if (imageFile) {
    ...
    // Ensure payment was successful before submitting the form
    if (!hasPaid) {
      alert("Please complete the payment before submitting.");
      return;
    }
  
    // Use the updated formData with the new photoUrl for the POST request
