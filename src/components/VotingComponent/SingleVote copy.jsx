...
//         if (votesResponse.status === 201) {
//           // Navigate only if the vote was added successfully
//           navigate(`/actors/${actorId}`, {
//             state: { returnPath: location.pathname },
//           });
//         } else {
//           // If the vote couldn't be added, set an error message
//           setErrorMessage(votesResponse.data.message || "Failed to add vote");
//           // Reset the voted state to allow retry
//           setVoted(false);
//         }
  ...
  const handleVoteClick = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Navigate to the actor's page after a successful vote
        navigate(`/actors/${actorId}`, {
          state: { returnPath: location.pathname },
        });
      } else if (response.status === 400) {
        setErrorMessage(response.data.message);
        console.error("Error while voting:", response.data.message);
        // Reset the voted state to allow retry
        setVoted(false);
      }
    } catch (error) {
